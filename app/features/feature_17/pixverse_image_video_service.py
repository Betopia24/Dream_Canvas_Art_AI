...
class PixverseImageVideoService:
    """Service for generating videos using Pixverse Image-to-Video from FAL.ai"""
    ...
    async def generate_video(self, prompt: str, user_id: str, image_file: UploadFile, shape: str) -> str:
        """
        Generate a video using Pixverse Image-to-Video and save it locally
        ...
        """
        try:
            logger.info(f"Generating video with Pixverse Image-to-Video for prompt: {prompt[:50]}...")
...
pixverse_image_video_service = PixverseImageVideoService()
